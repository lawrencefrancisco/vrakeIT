...
<body>
        ...
        <div class="page-body">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:16px;margin-bottom:24px;">
                <?php foreach (
                    ...
                    <div class="stat-card">
                        ...
                        <div style="font-size:1.75rem;font-weight:800;color:var(--text);"><?= $count ?></div>
                    </div>
                <?php endforeach; ?>
            </div>
            ...
            <div class="section-card">
                ...
                <div style="overflow-x:auto;">
                                ...
                                <tr data-status="<?= $status ?>"
                                    ...
                                    <td>
                                        ...
                                        <?php if ($c['party1_contact']): ?>
                                            <div style="font-size:11px;color:var(--muted);"><?= htmlspecialchars($c['party1_contact']) ?></div>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <div style="font-weight:600;"><?= htmlspecialchars($c['party2_name'] ?? '—') ?></div>
                                        <?php if ($c['party2_contact']): ?>
                                            <div style="font-size:11px;color:var(--muted);"><?= htmlspecialchars($c['party2_contact']) ?></div>
                                        <?php endif; ?>
                                    </td>
                                    <td style="text-transform:capitalize;font-size:12px;color:var(--muted);"><?= htmlspecialchars($c['parties'] ?? '—') ?></td>
                    ...
                    <div id="emptyMsg" style="display:none;text-align:center;padding:48px;color:var(--muted);">
    ...
    <div style="background:var(--muted);border-radius:12px;padding:4px 12px;margin-bottom:4px;">
      ...
      ${row('Party 1',       htmlEsc(c.party1_name ?? '—') + (c.party1_contact ? `<br><small style="color:var(--muted);">${htmlEsc(c.party1_contact)}</small>` : ''))}
      ${row('Party 2',       htmlEsc(c.party2_name ?? '—') + (c.party2_contact ? `<br><small style="color:var(--muted);">${htmlEsc(c.party2_contact)}</small>` : ''))}
